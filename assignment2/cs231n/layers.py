...
def affine_backward(dout, cache):
    ...
    dx = dx_row.reshape(x.shape) # (N, d1, ..., d_k)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, xhat, x_mean, x_var, gamma, beta, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == "test":
        ...
        cache = (x, xhat, running_mean, running_var, gamma, beta, eps)
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
    else:
...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dxhat * (x - mu) * -0.5 * (var + eps) ** (-1.5), axis=0)  # (D,)
    dmu = np.sum(dxhat * -1 / np.sqrt(var + eps), axis=0) + dvar * np.sum(-2 * (x - mu), axis=0) / N  # (D,)
    dx = dxhat / np.sqrt(var + eps) + dvar * 2 * (x - mu) / N + dmu / N # (N, D)
...
def batchnorm_backward_alt(dout, cache):
    ...
    np.sum(dxhat, axis=0) / (N * np.sqrt(var + eps)) - \
    xhat * np.sum(dxhat * xhat, axis=0) / (N * np.sqrt(var + eps))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    # still apply!                                                            #
    ###########################################################################
    x, xhat, mean, var, gamma, beta, eps = cache
    N, D = x.shape
    ...
    dx = dxhat / np.sqrt(var + eps) - \
    np.sum(dxhat, axis=1, keepdims=True) / (D * np.sqrt(var + eps)) - \
    xhat * np.sum(dxhat * xhat, axis=1, keepdims=True) / (D * np.sqrt(var + eps))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_mean = x_group.mean(axis=(2, 3, 4), keepdims=True)
    x_var = x_group.var(axis=(2, 3, 4), keepdims=True)
    xhat = (x_group - x_mean) / np.sqrt(x_var + eps)

    xhat = xhat.reshape(N, C, H, W)
    out = gamma * xhat + beta
    cache = (G, x, xhat, x_mean, x_var, gamma, beta, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    G, x, xhat, mean, var, gamma, beta, eps = cache 
    N, C, H, W = x.shape
    ...
    xhat_group = xhat.reshape(N, G, C//G, H, W)

    D = C // G * H * W

    dx_group = dxhat_group / np.sqrt(var + eps) - \
    np.sum(dxhat_group, axis=(2,3,4), keepdims=True) / (D * np.sqrt(var + eps)) - \
    xhat_group * np.sum(dxhat_group * xhat_group, axis=(2,3,4), keepdims=True) / (D * np.sqrt(var + eps))
